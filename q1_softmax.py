...
def softmax(x):
    ...
    ### YOUR CODE HERE
    x = x - tf.reshape(tf.reduce_max(x, axis=1), shape=[-1, 1])
    return tf.nn.softmax(x, dim=-1, name="softmax_probs")
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    y = tf.to_float(y)
    loss =  tf.reduce_sum(-1 * tf.mul(y, tf.log(yhat)), name="batch_loss")
    return loss
