...
from general_utils import logged_loop, get_minibatches, get_pickle
...
class Parser(object):
    ...
    def __init__(self, dataset, load_existing_vocab):
        ...
        if load_existing_vocab:
            self.tok2id = get_pickle("/home/asjindal/Downloads/assignment2/data/dump/word2idx.pkl")
            self.id2tok = {idx: token for (token, idx) in self.tok2id.items()}
            self.L_NULL = self.tok2id[P_PREFIX + NULL]
            self.P_UNK = self.tok2id[P_PREFIX + UNK]
            self.P_NULL = self.tok2id[P_PREFIX + NULL]
            self.P_ROOT = self.tok2id[P_PREFIX + ROOT]
            self.UNK = self.tok2id[UNK]
            self.NULL = self.tok2id[NULL]
            self.ROOT = self.tok2id[ROOT]
            print "loaded existing Vocab!"
        else:
            # tok2id = {P_PREFIX + l: i for (i, l) in enumerate(deprel)}
            tok2id = {}
            tok2id[P_PREFIX + NULL] = self.L_NULL = len(tok2id)
    ...
    def vectorize(self, examples):
        ...
        for ex in examples:
            ...
            head = [-1] + ex['head']
            label = [-1] + [self.tok2id[P_PREFIX + w] if P_PREFIX + w in self.tok2id else -1 for w in ex['label']]
...
def load_and_preprocess_data(reduced=True, load_existing_vocab = False):
    config = Config()
    ...
    start = time.time()
    parser = Parser(train_set, load_existing_vocab)
    print "took {:.2f} seconds".format(time.time() - start)
    ...
    # embeddings_matrix = np.asarray(np.random.uniform(
    #     low=0., high=0., size=(parser.n_tokens, 50)), dtype=np.float32)
    ...
    print sorted(parser.tok2id.keys())
