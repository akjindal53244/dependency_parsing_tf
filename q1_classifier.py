...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.input = tf.placeholder(shape=[self.config.batch_size, self.config.n_features], dtype=tf.float32, name="batch_input")
        self.targets = tf.placeholder(shape=[self.config.batch_size, self.config.n_classes], dtype=tf.int32, name = "batch_output")
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None):
        ...
        feed_dict = {self.input : inputs_batch, self.targets: labels_batch}
        return feed_dict
    ...
    def add_prediction_op(self):
        ...
        """
        w1 = tf.get_variable(shape=[self.config.n_features, self.config.n_classes],
                                 initializer=tf.random_uniform_initializer(minval=-0.1, maxval=0.1, dtype=tf.float32), trainable=True, name = "w1")
        b1 = tf.get_variable(shape=[self.config.n_classes],
                             initializer=tf.random_uniform_initializer(minval=-0.1, maxval=0.1, dtype=tf.float32), trainable=True, name = "b1")
        pred = softmax(tf.matmul(self.input, w1) + b1)
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss = cross_entropy_loss(self.targets, pred)
        ### END YOUR CODE
        return loss
